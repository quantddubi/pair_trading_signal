"""
Streamlit 페어트레이딩 분석 앱
유클리드 거리 기반 페어트레이딩 신호를 시각화하고 분석하는 도구
"""
...
# 모듈 import
common_utils = import_module_from_file("common_utils.py", "common_utils")
...
def load_price_data():
    """가격 데이터 로딩"""
    file_path = "/Users/a/PycharmProjects/pair_trading_signal/MU Price(BBG).csv"
...
def create_pair_chart(prices, asset1, asset2, formation_days, signal_days, asset_mapping=None):
    ...
    if asset_mapping:
        name1 = asset_mapping.get(asset1, asset1)
        name2 = asset_mapping.get(asset2, asset2)
        chart_title = f"페어트레이딩 분석: {asset1}({name1}) - {asset2}({name2})"
    else:
...
def main():
    ...
    **핵심 원리**: 가격 움직임의 경로가 가장 유사한 자산들을 페어로 선정하여 평균회귀를 노리는 전략입니다.
    
    **상세 작동 과정**
    1. **가격 정규화**
    2. **유클리드 거리 계산**: 모든 자산 쌍에 대해 정규화된 가격 경로 간 유클리드 거리 측정  
       - 거리 공식: √Σ(Asset1ᵢ - Asset2ᵢ)² (i=1부터 756일까지)
