...
class SSDDistancePairTrading:
    ...
    def calculate_ssd(self, series1: pd.Series, series2: pd.Series) -> float:
        ...
        s1_common = series1[common_idx]
        s2_common = series2[common_idx]
        
        # 정규화 (첫날 = 1)
        s1_normalized = s1_common / s1_common.iloc[0] if s1_common.iloc[0] != 0 else s1_common
        s2_normalized = s2_common / s2_common.iloc[0] if s2_common.iloc[0] != 0 else s2_common
